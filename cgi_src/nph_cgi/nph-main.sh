...
format_body()
{
	...
	if [ "$userCookie" != "" ]; then
		...
		BODY+="<form method=\"POST\" action=\"/cgi_src/nph_cgi/nph-loginAction.sh\" style=\"display: flex; flex-direction: column; max-width:20em; margin: auto; padding: 1em; border: solid black 1px\">"$LINEEND;
